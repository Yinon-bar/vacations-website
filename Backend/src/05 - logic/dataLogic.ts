...
async function addVacation(vacation: VacationModel): Promise<VacationModel> {
  if (vacation.image) {
    ...
    await vacation.image.mv("./src/01 - assets/images/" + vacation.image.name);
  }
  const sql = `
  INSERT INTO 
  vacations 
  VALUES(DEFAULT, '${vacation.description}', '${vacation.destination}', '${vacation.image.name}', '${vacation.start_date}', '${vacation.end_date}', ${vacation.price}, ${vacation.followers});
  `;
  const data = await dal.execute(sql);
  return data;
}
